...
class ContentComponentPro::Toolbar : public Component,
                                  public Button::Listener,
                                  public Timer
{
public:
    Toolbar (ContentComponentPro& o)
        : owner(o), viewBtn ("e")
    {
...
class ContentComponentPro::StatusBar : public Component,
...
{
public:
    ContentContainer (ContentComponentPro& cc, AppController& app)
        : owner (cc)
    {
...
private:
    friend class ContentComponentPro;
    ContentComponentPro& owner;
...
class ContentComponentPro::Resizer : public StretchableLayoutResizerBar
{
public:
    Resizer (ContentComponentPro& ContentComponentPro, StretchableLayoutManager* layoutToUse,
             int itemIndexInLayout, bool isBarVertical)
    : StretchableLayoutResizerBar (layoutToUse, itemIndexInLayout, isBarVertical),
      owner (ContentComponentPro)
    { }
...
struct ContentComponentPro::Tooltips
{
    Tooltips() { tooltipWindow = new TooltipWindow(); }
    ScopedPointer<TooltipWindow> tooltipWindow;
};

ContentComponentPro::ContentComponentPro (AppController& ctl_)
    : controller (ctl_)
{
...
Globals& ContentComponentPro::getGlobals() { return controller.getGlobals(); }
SessionPtr ContentComponentPro::getSession() { return getGlobals().getSession(); }

String ContentComponentPro::getMainViewName() const
{
...
String ContentComponentPro::getAccessoryViewName() const
{
...
int ContentComponentPro::getNavSize()
{
    return nav != nullptr ? nav->getWidth() : 220;
}

void ContentComponentPro::childBoundsChanged (Component* child) { }
void ContentComponentPro::mouseDown (const MouseEvent& ev) { Component::mouseDown (ev); }

void ContentComponentPro::setMainView (const String& name)
{
    if (name == "PatchBay") {
...
void ContentComponentPro::backMainView()
{
    setMainView (lastMainView.isEmpty() ? "GraphEditor" : lastMainView);
}

void ContentComponentPro::nextMainView()
{
...
void ContentComponentPro::setContentView (ContentView* view, const bool accessory)
{
...
void ContentComponentPro::setAccessoryView (const String& name)
{
    if (name == "PatchBay") {
...
void ContentComponentPro::paint (Graphics &g)
{
    g.fillAll (LookAndFeel::backgroundColor);
}

void ContentComponentPro::resized()
{
    Rectangle<int> r (getLocalBounds());
...
bool ContentComponentPro::isInterestedInDragSource (const SourceDetails& dragSourceDetails)
{
...
void ContentComponentPro::itemDropped (const SourceDetails& dragSourceDetails)
{
    const auto& desc (dragSourceDetails.description);
...
bool ContentComponentPro::isInterestedInFileDrag (const StringArray &files)
{
...
void ContentComponentPro::filesDropped (const StringArray &files, int x, int y)
{
    for (const auto& path : files)
    {
        const File file (path);
...
void ContentComponentPro::post (Message* message)
{
...
void ContentComponentPro::saveState (PropertiesFile* props)
{
    jassert (props);
    if (nav)
        nav->saveState (props);
...
void ContentComponentPro::restoreState (PropertiesFile* props)
{
    jassert (props);
    if (nav)
...
void ContentComponentPro::setCurrentNode (const Node& node)
{
...
void ContentComponentPro::updateLayout()
{
...
void ContentComponentPro::resizerMouseDown()
{
    updateLayout();
}

void ContentComponentPro::resizerMouseUp()
{
...
void ContentComponentPro::setNodeChannelStripVisible (const bool isVisible)
{
    if (! nodeStrip)
    {
...
bool ContentComponentPro::isNodeChannelStripVisible() const { return nodeStrip && nodeStrip->isVisible(); }
...
ApplicationCommandTarget* ContentComponentPro::getNextCommandTarget()
{
    return (container) ? container->content1.get() : nullptr;
}

void ContentComponentPro::setShowAccessoryView (const bool show)
{
    if (container) container->setShowAccessoryView (show);
}

bool ContentComponentPro::showAccessoryView() const
{
