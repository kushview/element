...
void MediaManager::closeFile (const File& f, bool saveIfNeeded)
{
    for (int i = documents.size(); --i >= 0;)
    {
        Document* d = documents.getUnchecked (i);
...
bool MediaManager::closeAllDocumentsUsingSession (Session& session, bool saveIfNeeded)
{
    for (int i = documents.size(); --i >= 0;)
    {
        Document* d = documents.getUnchecked (i);
...
void RecentDocumentList::newDocumentOpened (MediaManager::Document* document)
{
