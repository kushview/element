...
struct ScriptNode::Context
{
    ...
    ~Context()
    {
        for (auto* ip : inParams)
            dynamic_cast<ScriptNodeParameter*>(ip)->unlink();
        for (auto* op : outParams)
            dynamic_cast<ScriptNodeParameter*>(op)->unlink();
        inParams.clear();
        outParams.clear();
    ...
    void setParameterData (const MemoryBlock& block)
    {
        ...
        for (int i = 0; i < inParams.size(); ++i)
            if (auto* param = dynamic_cast<ScriptNodeParameter*> (inParams.getObjectPointerUnchecked (i)))
    ...
    void copyParameterValues (const Context& other)
    {
        ...
        {
            auto* const param = dynamic_cast<ScriptNodeParameter*> (ip);
...
private:
    ...
    ScriptNodeParameter* findParameter (const PortDescription& port) const
    {
        for (auto* const ip : inParams)
        {
            auto* const param = dynamic_cast<ScriptNodeParameter*> (ip);
        ...
        if (sol::function f = state ["node_params"])
        {
            ...
            try
            {
                ...
                for (size_t i = 0; i < params.size(); ++i)
                {
                    ...
                    lparams.add (new ScriptNodeParameter (this, ports.getPort (ports.size() - 1)));
                }
