        ...
        for (int ch = 0; ch < numChans; ++ch)
        {
            ...
            for (int i = 0; i < numSamples; ++i)
            {
                ...
                switch (waveType)
                {
                    ...
                    case Sine:
                        sample = std::sin (phase * MathConstants<double>::twoPi);
                        break;
                ...
                output[i] = sample * amp * (1.0f + ch * 0.1f); // Slight variation per channel
            }
...
public:
    ...
    void render (RenderContext& rc) override
    {
        ...
        // In a 3-channel buffer with 3 inputs + 1 output, GraphNode may map them creatively
        // We'll use a simple strategy: read from first N input channels, write to last channel
...
BOOST_AUTO_TEST_CASE (AudioStereoRouting)
{
    ...
    // Verify both channels received audio with different characteristics
    BOOST_REQUIRE (dst->hasAudio());
    BOOST_REQUIRE_GT (dst->getPeakLevel (0), 0.9f);
    BOOST_REQUIRE_GT (dst->getPeakLevel (1), 0.9f);
    // Channel 1 should have slightly higher amplitude due to generator design
    BOOST_REQUIRE_GT (dst->getPeakLevel (1), dst->getPeakLevel (0));
}
...
BOOST_AUTO_TEST_CASE (AudioChannelCrossover)
{
    ...
    // Verify crossover worked - channel 0 should have gen's channel 1 characteristics
    BOOST_REQUIRE (dst->hasAudio());
    const float peak0 = dst->getPeakLevel (0);
    const float peak1 = dst->getPeakLevel (1);
    BOOST_REQUIRE_GT (peak0, 1.0f);  // Received gen channel 1 (higher amp)
    BOOST_REQUIRE_CLOSE (peak1, 1.0f, 0.05f);  // Received gen channel 0
}

#if 0 // TODO: AudioMixerNode test disabled - GraphNode buffer allocation doesn't match simple layout assumptions
